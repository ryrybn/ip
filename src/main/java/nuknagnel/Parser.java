...
public class Parser {
  public static ParsedCommand parse(String input) {
    ...
    switch (command) {
      ...
      case "find":
        return new ParsedCommand(ParsedCommand.Type.FIND, parseKeyword(rest));
      default:
        throw new InvalidInputException();
    }
  }
  ...
  private static String parseKeyword(String raw) {
    String keyword = raw == null ? "" : raw.trim();
    if (keyword.isEmpty()) {
      throw new InvalidInputException("Find command must include a keyword.");
    }
    return keyword;
  }
}
